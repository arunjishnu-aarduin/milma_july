    ...
    for method in method_list:
        ...
        for composition in composition_list:
            ...
            if composition.issue == requested_issue:
                ...
                target_unit += composition.ratio * sale_in_unit * (
                getMethodPercentage(composition.category, composition.method) / 100)
...
def totalIssueRequirement(month, diary, issue):
    ...
    for category in categoryList:
        ...
        saledetails = ActualSale.objects.filter(product__in=(Product.objects.filter(category=category)).values('code'),
                                                month=month, diary=diary)

        stockindetail = ActualStockin.objects.filter(
            product__in=(Product.objects.filter(category=category)).values('code'), month=month, diary=diary)

        stockoutdetail = ActualStockin.objects.filter(
            product__in=(Product.objects.filter(category=category)).values('code'), month=month, from_diary=diary)
...
def qsmpValue(milk_product):
    try:
        ...
        qsmp = qwm * (
        (((ms - swm) * (mf - fc)) - ((ms - sc) * (mf - fwm))) / (((mf - fsmp) * (ms - sc)) - ((ms - ssmp) * (mf - fc))))
...
def qsmpReconstitutionValueWMP():
    try:
        ...
        qsmp = qw * (
        ((fwm * (swm - swmp)) - (swm * (fwm - fwmp))) / (((fwm - fsmp) * (swm - swmp)) - ((fwm - fwmp) * (swm - ssmp))))
    ...
    for diary in diaryList:
        ...
        for item in resultitem:
            if diary.name == item['fromdiary'] or diary.name == item['todiary']:
                ...
                result = {"diary": diary.name, "type": transfer_type, "to_or_from_diary": to_or_from_diary,
                          "value": item['value']}
...
def wmBalancing(after_transfer_diary_list, month):
    ...
    for item in after_transfer_diary_list:
        ...
        if item['type'] == "Shortage":
            ...
            if reconstitution_amount != 0:
                ...
                try:
                    ...
                    wmp_ratio_wmp = qwmpReconstitutionValueWMP() / (
                    qw + qwmpReconstitutionValueWMP() - qsmpReconstitutionValueWMP())
                    smp_ratio_wmp = (-1 * qsmpReconstitutionValueWMP()) / (
                    qw + qwmpReconstitutionValueWMP() - qsmpReconstitutionValueWMP())
            ...
            transaction_item = {
                'transaction': " Quantity of Cream Obtained in CSM Conversion:" + str(
                    pcsm_for_conversion * csm_cream_ratio)}
        ...
        resultitem = {'diary': item['diary'], 'after_transfer': str(item['type']) + " : " + str(item['value']),
                      'transactions': wm_after_stock_transfer, 'wmp_used': wmp_total_wmp_reconstitution
            , 'smp_used_smp': smp_total_smp_reconstitution, 'smp_used_wmp': smp_total_wmp_reconstitution,
                      'converted_smp': csm_converted_to_smp, 'cream_used_csm': cream_total_csm
            , 'cream_used_smp': cream_total_smp_reconstitution, 'wm_used_csm': wm_total_csm,
                      'water_used_wmp': water_total_wmp_reconstitution
...
def creamBalancing(after_transfer_diary_list):
    ...
    for item in after_transfer_diary_list:
        ...
        if item['type'] == "Shortage":
            try:
                cream_after_stock_transfer = " Cream Purchased:" + str(after_transfer_value) + " ,Amount:" + str(
                    after_transfer_value * GeneralCalculation.objects.get(code=15).value)
            except Exception as e:
                print "Exception Handled At 848"
            cream_after_stock_transfer += ""
        else:
            try:
                cream_after_stock_transfer = " Cream sold:" + str(after_transfer_value) + " ,Amount:" + str(
                    after_transfer_value * GeneralCalculation.objects.get(code=16).value)
            except Exception as e:
                print "Exception Handled At 848"
            cream_after_stock_transfer += ""

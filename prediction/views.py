...
def issuerequirement(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(request, "prediction/IssueWise.html",
                          {"issue_monthwise": issue_monthwise, 'form': form, 'issue_as_product': issue_as_product,
                           'issue_as_issue': issue_as_issue})
...
def basicRequirement(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(request, "prediction/IssueWiseBasic.html",
                          {"issue_monthwise": total_requirement, 'form': form})
...
def basicRequirementUnion(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(request, "prediction/IssueWiseBasicUnion.html",
                          {"total_requirement_union": total_requirement_union, 'form': form})
...
def basicRequirementUnionDiaryWise(request):
    if request.method == "POST":
        ...
        if form_union.is_valid():
            ...
            return render(request, "prediction/IssueWiseBasicUnionDiaryWise.html",
                          {"issue_monthwise": total_requirement, 'form_union': form_union})
    ...
    if request.method == "POST":
        ...
        if request.POST.get('delete', False):
            ...
            if form.is_valid():
                ...
                try:
                    Variant.objects.get(name=str(form.cleaned_data["first_name"]) + str(form.cleaned_data["unit_name"]),
                                        unit=form.cleaned_data["unit"]).delete()
    ...
    if request.method == "POST":
        ...
        if request.POST.get('delete', False):
            if form.is_valid():
                ...
                try:
                    Product.objects.get(code=form.cleaned_data["code"], category=form.cleaned_data["category"],
                                        variant=form.cleaned_data["variant"]).delete()
                    messages.info(request, "Successfully Deleted")
                except Exception as e:
                    messages.info(request, "Deletion Failed:Not Exist")
        ...
        if request.POST.get('delete', False):
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    Composition.objects.get(category=form.cleaned_data["category"], method=form.cleaned_data["method"],
                                            issue=form.cleaned_data["issue"]).delete()
                    messages.info(request, "Successfully Deleted")
                # try:
                # 	Config_Attribute_obj = ConfigurationAttribute.objects.first()
...
def methodpercentageNew(request):
        ...
        if request.POST.get('delete', False):
            ...
            if form.is_valid():
                try:
                    ...
                    MethodPercentage.objects.get(category=form.cleaned_data["category"],
                                                 method=form.cleaned_data["method"]).delete()
                    messages.info(request, "Successfully Deleted")
                # try:
                # 	Config_Attribute_obj = ConfigurationAttribute.objects.first()
                # 	Config_Attribute_obj.issue_requirement_change_status = True
    ...
    return render(request, 'prediction/MethodPercentage.html',
                  {'form': form, 'method_percentage_List': method_percentage_List})
...
def fatPercentageYield(request):
        ...
        if request.POST.get('delete', False):
            ...
            if form.is_valid():
                try:
                    ...
                    FatPercentageYield.objects.get(category=form.cleaned_data["category"],
                                                   issue=form.cleaned_data["issue"],
                                                   method=form.cleaned_data["method"]).delete()
                    messages.info(request, "Successfully Deleted")
                # try:
    ...
    return render(request, 'prediction/FatPercentageYield.html',
                  {'form': form, 'fat_percentage_yield_List': fat_percentage_yield_List})
...
def targetYear(request):
    ...
    actualProductSales = ActualSale.objects.filter(diary=diary,year=request.session['session_current_year']).order_by('month')
    actualProductStockIn = ActualStockin.objects.filter(diary=diary,year=request.session['session_current_year']).order_by('month')
    actualProductStockOut = ActualStockin.objects.filter(from_diary=diary,year=request.session['session_current_year']).order_by('month')
    targetWMProcurement = ActualWMProcurement.objects.filter(diary=diary,year=request.session['session_current_year']).order_by('month')
    return render(request, 'prediction/Targetyear.html',
                  {'actualProductSales': actualProductSales, 'actualProductStockIn': actualProductStockIn,
                   'actualProductStockOut': actualProductStockOut, 'targetWMProcurement': targetWMProcurement})


@login_required
@user_passes_test(group_check_union)
def targetYearUnion(request):
    actualProductSales = ActualSale.objects.filter(year=request.session['session_current_year']).order_by('month')
    actualProductStockIn = ActualStockin.objects.filter(year=request.session['session_current_year']).order_by('month')
    actualProductStockOut = ActualStockin.objects.filter(year=request.session['session_current_year']).order_by('month')
    targetWMProcurement = ActualWMProcurement.objects.filter(year=request.session['session_current_year']).order_by('month')
    return render(request, 'prediction/TargetyearUnion.html',
                  {'actualProductSales': actualProductSales, 'actualProductStockIn': actualProductStockIn,
                   'actualProductStockOut': actualProductStockOut, 'targetWMProcurement': targetWMProcurement})
...
def growthFactorEntry(request):
    diary = diary_of_user(request.user)
    productGrowthFactor = ProductCategoryGrowthFactor.objects.filter(diary=diary,year=request.session['session_current_year']).order_by('month', 'category')
    procurementGrowthFactor = ProcurementGrowthFactor.objects.filter(diary=diary,year=request.session['session_current_year']).order_by('month', 'diary')
    ...
    if request.method == "POST":
        ...
        #region Procurement GF Deletion
        if request.POST.get('delete', False):

            if form_procurement.is_valid():
                try:
                    ProcurementGrowthFactor.objects.get(month=form_procurement.cleaned_data["month"],
                                                        diary=diary,year=request.session['session_current_year']).delete()
                    messages.info(request, "Successfully Deleted")
                except Exception as e:
                    messages.info(request, "Deletion Failed:Not Exist")
        ...
        #region Product GF Deletion
        if request.POST.get('delete_product_growth_factor', False):

            if form.is_valid():
                try:
                    ProductCategoryGrowthFactor.objects.get(month=form.cleaned_data["month"],year=request.session['session_current_year'], diary=diary,
                                                            category=form.cleaned_data["category"]).delete()
                    messages.info(request, "Successfully Deleted")
        ...
        #region Product GF Insertion
        if form.is_valid():
            data = form.cleaned_data
            obj, created = ProductCategoryGrowthFactor.objects.update_or_create(
                diary=diary, month=data['month'], category=data['category'],year=request.session['session_current_year'],
        ...
        #Procurement GF Insertion
        if form_procurement.is_valid():
            data = form_procurement.cleaned_data
            obj, created = ProcurementGrowthFactor.objects.update_or_create(
                diary=diary, month=data['month'],year=request.session['session_current_year'],
                defaults={'growth_factor': data['growth_factor']},
    ...
    return render(request, 'prediction/GrowthFactorEntry.html',
                  {'form_procurement': form_procurement, 'form': form, 'productGrowthFactor': productGrowthFactor,
                   'procurementGrowthFactor': procurementGrowthFactor})
...
def growthFactorEntryUnion(request):
    productGrowthFactor = ProductCategoryGrowthFactor.objects.filter(year=request.session['session_current_year']).order_by('month', 'category')
    procurementGrowthFactor = ProcurementGrowthFactor.objects.filter(year=request.session['session_current_year']).order_by('month', 'diary')
    if request.method == "POST":
        ...
        if request.POST.get('delete', False):
            ...
            if form_procurement.is_valid():
                try:
                    ProcurementGrowthFactor.objects.get(month=form_procurement.cleaned_data["month"],year=request.session['session_current_year'],
                                                        diary=form_procurement.cleaned_data["diary"]).delete()
                    messages.info(request, "Successfully Deleted")
                except Exception as e:
                    messages.info(request, "Deletion Failed:Not Exist")
        ...
        if request.POST.get('delete_product_growth_factor', False):
            ...
            if form.is_valid():
                try:
                    ProductCategoryGrowthFactor.objects.get(month=form.cleaned_data["month"],
                                                            diary=form.cleaned_data["diary"],year=request.session['session_current_year'],
                                                            category=form.cleaned_data["category"]).delete()
                    messages.info(request, "Successfully Deleted")
        ...
        if form.is_valid():
            data = form.cleaned_data
            obj, created = ProductCategoryGrowthFactor.objects.update_or_create(
                diary=data['diary'], month=data['month'], category=data['category'],year=request.session['session_current_year'],
                defaults={'growth_factor': data['growth_factor']},
        ...
        if form_procurement.is_valid():
            ...
            obj, created = ProcurementGrowthFactor.objects.update_or_create(
                diary=data['diary'], month=data['month'],year=request.session['session_current_year'],
                defaults={'growth_factor': data['growth_factor']},
    ...
    # return HttpResponseRedirect(request,'prediction/GrowthFactorEntry.html',{'form_procurement':form_procurement,'procurementGrowthFactor':procurementGrowthFactor})
        ...
        # return render(request,'prediction/all_new.html',{'form':form,'name':"Product Category Growth Factor"})
    return render(request, 'prediction/GrowthFactorEntryUnion.html',
                  {'form_procurement': form_procurement, 'form': form, 'productGrowthFactor': productGrowthFactor,
                   'procurementGrowthFactor': procurementGrowthFactor})
...
def productConfiguration(request):
        ...
        if request.POST.get('delete', False):
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    ProductConfiguration.objects.get(product=form.cleaned_data["product"],
    ...
    return render(request, 'prediction/ProductConfiguration.html',
                  {'form': form, 'productConfigurationList': productConfigurationList})
...
def actualYearEntry(request):
    ...
    ActualWMProcurementList = ActualWMProcurement.objects.filter(diary=diary,year=request.session['session_current_year']).order_by('month')
    ActualSaleList = ActualSale.objects.filter(diary=diary,year=request.session['session_current_year']).order_by('month')
    ActualStockinList = ActualStockin.objects.filter(diary=diary,year=request.session['session_current_year']).order_by('month')
    ActualStockoutList = ActualStockin.objects.filter(from_diary=diary,year=request.session['session_current_year']).order_by('month')
    ...
    if request.method == "POST":
        ...
        if request.POST.get('delete', False):
            ...
            if form.is_valid():
                ...
                try:
                    ActualWMProcurement.objects.get(diary=diary, month=form.cleaned_data["month"],year=request.session['session_current_year']).delete()
                    messages.info(request, "Successfully Deleted")
                except Exception as e:
                    messages.info(request, "Deletion Failed:Not Exist")
        ...
        if request.POST.get('delete_sale', False):
            ...
            if form_sale.is_valid():
                ...
                try:
                    ActualSale.objects.get(diary=diary, month=form_sale.cleaned_data["month"],year=request.session['session_current_year'],
                                           product=form_sale.cleaned_data["product"]).delete()
                    messages.info(request, "Successfully Deleted")
        ...
        if form_sale.is_valid():
            ...
            obj, created = ActualSale.objects.update_or_create(
                diary=diary, month=data['month'],year=request.session['session_current_year'], product=data["product"],
        ...
        if request.POST.get('delete_stockin', False):
            ...
            if form_stockin.is_valid():
                ...
                try:
                    ActualStockin.objects.get(diary=diary, from_diary=form_stockin.cleaned_data["from_diary"],
                                              month=form_stockin.cleaned_data["month"],year=request.session['session_current_year'],
                                              product=form_stockin.cleaned_data["product"]).delete()
                    # t = Thread(target=totalIssueRequirementDB,
        ...
        if form_stockin.is_valid():
            ...
            product_config_check = ProductConfiguration.objects.filter(diary=data['from_diary'],
                                                                       product=data["product"])

            if product_config_check.count() > 0:

                obj, created = ActualStockin.objects.update_or_create(
                    diary=diary, from_diary=data['from_diary'], month=data['month'],year=request.session['session_current_year'], product=data["product"],
        ...
        form_sale.fields['product'].queryset = Product.objects.filter(
            code__in=(ProductConfiguration.objects.filter(diary=diary).values('product'))).all()

        form_stockin = ActualStockinForm()
        form_stockin.fields['product'].queryset = Product.objects.filter(
            code__in=(ProductConfiguration.objects.filter(diary=diary).values('product'))).all()
    return render(request, 'prediction/ActualYearEntry.html',
                  {'form': form, 'ActualWMProcurementList': ActualWMProcurementList, 'form_sale': form_sale,
                   'ActualSaleList': ActualSaleList, 'form_stockin': form_stockin,
                   'ActualStockinList': ActualStockinList, 'ActualStockoutList': ActualStockoutList})


@login_required
@user_passes_test(group_check_union)
def actualYearEntryUnion(request):
    ActualWMProcurementList = ActualWMProcurement.objects.filter(year=request.session['session_current_year']).order_by('month')
    ActualSaleList = ActualSale.objects.filter(year=request.session['session_current_year']).order_by('month')
    ActualStockinList = ActualStockin.objects.filter(year=request.session['session_current_year']).order_by('month')
    ActualStockoutList = ActualStockin.objects.filter(year=request.session['session_current_year']).order_by('month')
    if request.method == "POST":
        ...
        if request.POST.get('delete', False):
            ...
            if form.is_valid():
                ...
                try:
                    ActualWMProcurement.objects.get(diary=form.cleaned_data["diary"],
                                                    month=form.cleaned_data["month"],year=request.session['session_current_year']).delete()
                    messages.info(request, "Successfully Deleted")
                except Exception as e:
                    messages.info(request, "Deletion Failed:Not Exist")
        ...
        if request.POST.get('delete_sale', False):
            ...
            if form_sale.is_valid():
                ...
                try:
                    ActualSale.objects.get(diary=form_sale.cleaned_data["diary"], month=form_sale.cleaned_data["month"],year=request.session['session_current_year'],
                                           product=form_sale.cleaned_data["product"]).delete()
                    # t = Thread(target=totalIssueRequirementDB,
        ...
        if request.POST.get('delete_stockin', False):
            ...
            if form_stockin.is_valid():
                ...
                try:
                    ActualStockin.objects.get(diary=form_stockin.cleaned_data["diary"],
                                              from_diary=form_stockin.cleaned_data["from_diary"],
                                              month=form_stockin.cleaned_data["month"],year=request.session['session_current_year'],
                                              product=form_stockin.cleaned_data["product"]).delete()
                    # t = Thread(target=totalIssueRequirementDB,
        ...
        if form_stockin.is_valid():
            data = form_stockin.cleaned_data
            obj, created = ActualStockin.objects.update_or_create(
                diary=data['diary'], from_diary=data['from_diary'], month=data['month'],year=request.session['session_current_year'], product=data["product"],
                defaults={'quantity': data['quantity']},
    ...
    return render(request, 'prediction/ActualYearEntryUnion.html',
                  {'form': form, 'ActualWMProcurementList': ActualWMProcurementList, 'form_sale': form_sale,
                   'ActualSaleList': ActualSaleList, 'form_stockin': form_stockin,
                   'ActualStockinList': ActualStockinList, 'ActualStockoutList': ActualStockoutList})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if productlist_of_category.exists():
                ...
                product_sales_list = ActualSale.objects.filter(product__in=productlist_of_category, month=month,
                                                               diary=diary)
                ...
                if len(product_sales_list) == 0:
                    ...
                    print "No Sales Available for " + str(category.name) + " in " + str(
                        MONTHS[month]) + " for Diary :" + str(diary.name)
                else:
                    for sale in product_sales_list:
                        target_sale_unit_of_category += sale.targetSalesUnit
                print "sale-", target_sale_unit_of_category

                product_stockin_list = ActualStockin.objects.filter(product__in=productlist_of_category, month=month,
                                                                    diary=diary)
                target_stockin_unit_of_category = 0
                if len(product_stockin_list) == 0:
                    # return HttpResponse("No Sales Available for "+str(category.name)+" in "+str(MONTHS[month])+" for Diary :"+str(diary.name))
                    print "No Stockin Available for " + str(category.name) + " in " + str(
                        MONTHS[month]) + " for Diary :" + str(diary.name)
                else:
                ...
                product_stockout_list = ActualStockin.objects.filter(product__in=productlist_of_category, month=month,
                                                                     from_diary=diary)
                target_stockout_unit_of_category = 0
                if len(product_stockout_list) == 0:
                    # return HttpResponse("No Sales Available for "+str(category.name)+" in "+str(MONTHS[month])+" for Diary :"+str(diary.name))
                    print "No Stockout Available for " + str(category.name) + " in " + str(
                        MONTHS[month]) + " for Diary :" + str(diary.name)
                else:
                ...
                target_unit = (
                              target_sale_unit_of_category + target_stockout_unit_of_category) - target_stockin_unit_of_category
                ...
                if target_unit == 0:
                    ...
                    messages.info(request,
                                  "Netvalue is zero(Stockout+Sale-Stockin=0) for " + str(category.name) + " in " + str(
                                      MONTHS[month]) + " for Diary :" + str(diary.name))
                ...
                if len(method_list) == 0:
                    ...
                    messages.info(request, "No production methods are available for this category:" + str(
                        category.name) + " ,Please Enter Values in Composition ")
                    ...
                    for method in method_list:
                        ...
                        portion_of_target_unit_in_gram_through_this_method = target_unit_in_gram * (
                        methodPercentage / 100)

                        issues = []
                        resultitem = {"method": method,
                                      "portion_through_the_method": portion_of_target_unit_in_gram_through_this_method,
                                      "method_percentage": methodPercentage, "issue_required": issues}

                        composition_list = Composition.objects.filter(category=category, method=method['method'])

                        for composition in composition_list:
                            issues = [composition.issue.name,
                                      portion_of_target_unit_in_gram_through_this_method * composition.ratio]
                    ...
                    return render(request, "prediction/RawmaterialWise.html",
                                  {'result': result, 'form': form, 'total': total, 'target_unit': target_unit})
            else:
        ...
        if form_union.is_valid():
            ...
            if productlist_of_category.exists():
                ...
                if len(method_list) == 0:
                    ...
                    return render(request, "prediction/RawmaterialWiseUnion.html",
                                  {'result': result, 'form': form_union, 'total': total})
            else:
                # return HttpResponse("You have not configured any product for category:"+category.name)
                messages.info(request, "You have not configured any product for category:" + category.name)
    else:
...
def issuerequirementUnion(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(request, "prediction/IssueWiseUnion.html",
                          {"issue_monthwise": issue_monthwise, 'form': form, 'issue_as_product': issue_as_product,
                           'issue_as_issue': issue_as_issue})
        ...
        if form.is_valid():
            ...
            for diary in diary_list:
                ...
                if difference != 0:
                    ...
                    result = {"month": MONTHS[month], "diary": diary.name, "requirement": total_month_requirement_wm,
                              "procurement": total_month_procurement, "difference": difference,
                              "type": type_of_difference}
            ...
            for item_cream in cream_requirement_list:
                ...
                for item in after_wm_balancing_diarylist:
                    ...
                    if item_cream['diary'].name == item['diary']:
                        ...
                        item_cream['total_cream_used'] = 0 - (
                        requirement + item['cream_used_csm'] + item['cream_used_smp'])
            ...
            return render(request, 'prediction/Balancing.html',
                          {'form': form, 'resultitem': resultitem, 'interstock': inter_stock_transfer,
                           'after_transfer': wm_after_transfer

                              , 'cream_list': cream_list, 'inter_stock_cream': inter_stock_transfer_cream,
                           'cream_after_transfer': cream_after_transfer
                              , 'smp_list': smp_requirement_list, 'smp_after_transfer': smp_after_transfer,
                           'wmp_list': wmp_requirement_list, 'wmp_after_transfer': wmp_after_transfer})
...
def reportAnnual(request):
    ...
    for month_item in MONTHS.items():
        ...
        after_transfer_diary, inter_stock_transfer = interMilkTransfer(shortage_list, surplus_list,
                                                                       total_requirement_diary_cpd, diary_list)
        after_wm_balancing_diarylist, wm_reconstitution, wm_purchase, wm_sale, csm_sale, csm_converted, cream_reconstitution, converted_smp = wmBalancingReport(
            after_transfer_diary, month)
        ...
        targetYear = int(request.session['session_current_year']) + 1
    ...
    for category in CategoryList:
        ...
        for month in MONTHS.items():
            ...
            saledetails = ActualSale.objects.filter(
                product__in=(Product.objects.filter(category=category)).values('code'),
                month=month[0])
    ...
    return render(request, 'prediction/ReportAnnualTally.html',
                  {'total_milk_handled': total_milk_handled, 'month': month_list,
                   'wm_requirement_product': wm_requirement_product, 'total_sm_required': total_sm_required,
                   'total_milk_required_products': total_milk_required_products
                      , 'milk_diverted_percentage': milk_diverted_percentage, 'wm_surplus_sale': wm_surplus_sale,
                   'distress_milk_percentage': distress_milk_percentage
                      , 'csm_surplus_sale': csm_surplus_sale, 'csm_converted_smp': csm_converted_smp,
                   'total_cream_handled': total_cream_handled, 'total_cream_matric': total_cream_matric
                      , 'total_cream_required': total_cream_required, 'total_cream_purchased': total_cream_purchased,
                   'total_cream_sold': total_cream_sold
                      , 'distress_cream_percentage': distress_cream_percentage,
                   'wm_purchase_percentage': wm_purchase_percentage, 'total_smp_purchased': total_smp_purchased
                      , 'total_smp_sold': total_smp_sold, 'total_smp_recieved': total_smp_recieved,
                   'total_wmp_sold': total_wmp_sold,
        ...
        if form.is_valid():
            ...
            for item_smp in smp_requirement_list:
                ...
                if item_smp['type'] == "Sale":
                    try:
                        sale_rate = GeneralCalculation.objects.get(code='14').value
                        smp_item = {'transaction': 'SMP Sold:' + str(item_smp['total_smp_used']) + " ,Amount:" + str(
                            item_smp['total_smp_used'] * sale_rate), 'diary': item_smp['diary'].name}
            ...
            return render(request, 'prediction/BalancingUnion.html',
                          {'form': form, 'resultitem': resultitem, 'interstock': inter_stock_transfer,
                           'after_transfer': after_wm_balancing_diarylist

                              , 'cream_list': cream_list, 'inter_stock_cream': inter_stock_transfer_cream,
                           'cream_after_transfer': after_cream_balancing_diarylist
                              , 'smp_list': smp_requirement_list, 'smp_after_transfer': smp_after_transfer,
                           'wmp_list': wmp_requirement_list, 'wmp_after_transfer': wmp_after_transfer})
...
def financialYear(request):
    ...
    if request.method == "POST":
        ...
        if Year_Choice.count() == 0:
            ...
            form.fields['financial_Year'].choices = ((x[0], x[0]) for x in Year_Choice)
            session_current_year = request.session.get('session_current_year', current_year)

        else:

            form.fields['financial_Year'].choices = ((x['year'], x['year']) for x in Year_Choice)
            session_current_year = request.session.get('session_current_year', Year_Choice.last()['year'])
            form.fields['financial_Year'].initial = session_current_year

        next_year = int(session_current_year) + 1

    if (request.user.groups.filter(name='Union')):
        return render(request, 'prediction/FinancialYearUnion.html',
                      {'form': form, 'next_year': next_year, 'next_year_plus': next_year + 1})
    else:
        return render(request, 'prediction/FinancialYear.html',
                      {'form': form, 'next_year': next_year, 'next_year_plus': next_year + 1})
...
def reportMAT(request):
    ...
    for month_item in MONTHS.items():
            ...
            for issue_item in type2_issue_list:
                ...
                if diary.name == current_diary.name:
                    ...
                    if issue_item.name.upper() == "STD":
                        ...
                        cream_Jersey_production[month] = requirement_to_produce_milk_issue_cream * (-1)
                        ...
                        cream_tm_production[month] = requirement_to_produce_milk_issue_cream * (-1)
                        ...
                        cream_dtm_production[month] = requirement_to_produce_milk_issue_cream * (-1)
                        ...
                        cream_htm_production[month] = requirement_to_produce_milk_issue_cream * (-1)
                        ...
                        cream_sm_production[month] = requirement_to_produce_milk_issue_cream * (-1)
        ...
        after_cream_balancing_diarylist, cream_purchase, cream_sale = creamBalancingReportDiaryWise(
            after_cream_transfer_diary, current_diary.name)

        print str(smp_requirement_list)
        ...
        wm_output_total[
            month] = wm_requirement_product_month + wm_std_production_month + wm_Jersey_production_month + wm_tm_production_month + wm_htm_production_month + wm_dtm_production_month + wm_sm_production_month + wm_stockout_month + wm_sale + wm_for_csm
        ...
        Jersey_input_total[
            month] = wm_Jersey_production_month + cream_Jersey_production_month + smp_Jersey_production_month
        ...
        rc_input_total[
            month] = cream_reconstitution + smp_rc_smp + smp_rc_wmp + water_rc_smp + water_rc_wmp + wmp_rc_wmp
        rc_output_total[month] = wm_reconstitution_list[month]

        wm_surplus_csm[month] = wm_for_csm
        wm_surplus_csm_year += wm_for_csm
        cream_surplus_csm[month] = cream_for_csm * (-1)
        cream_surplus_csm_year += cream_for_csm

        cream_input_total[month] = (
                                   cream_purchase + cream_stockin_month + cream_Jersey_production_month + cream_tm_production_month + cream_htm_production_month + cream_dtm_production_month + cream_sm_production_month + cream_for_csm) * (
                                   -1)
        cream_output_total[
            month] = cream_std_production_month + cream_reconstitution + cream_requirement_product_month + cream_stockout_month + cream_sale
    ...
    wm_input_total[15] = wm_procurement_fat_year + wm_purchase_list[15] + wm_reconstitution_list[
        15] + wm_stockin_fat_year
    wm_input_total[16] = wm_snf
    wm_input_total[17] = wm_procurement_snf_year + wm_purchase_list[17] + wm_reconstitution_list[
        17] + wm_stockin_snf_year
    ...
    wm_output_total[
        13] = wm_requirement_product_year + wm_std_production_year + wm_Jersey_production_year + wm_htm_production_year + wm_dtm_production_year + wm_sm_production_year + wm_stock_out_year + wm_surplus_sale_year + wm_surplus_csm_year + wm_tm_production_year
    wm_output_total[14] = wm_fat
    wm_output_total[15] = wm_tm_production[15] + wm_stock_out_fat_year + wm_surplus_csm[15] + wm_surplus_sale_list[15] + \
                          wm_sm_production[15] + wm_dtm_production[15] + wm_htm_production[15] + wm_Jersey_production[
                              15] + wm_std_production[15] + wm_requirement_product[15]
    wm_output_total[16] = wm_snf
    wm_output_total[17] = wm_tm_production[17] + wm_stock_out_snf_year + wm_surplus_csm[17] + wm_surplus_sale_list[17] + \
                          wm_sm_production[17] + wm_dtm_production[17] + wm_htm_production[17] + wm_Jersey_production[
                              17] + wm_std_production[17] + wm_requirement_product[17]
    ...
    rc_input_total[
        13] = cream_rc_requirement_year + smp_rc_smp_requirement_year + smp_rc_wmp_requirement_year + water_rc_smp_requirement_year + water_rc_wmp_requirement_year + wmp_rc_wmp_requirement_year
    rc_input_total[14] = 0
    rc_input_total[15] = cream_rc_requirement[15] + wmp_rc_wmp_requirement[15]
    rc_input_total[16] = 0
    rc_input_total[17] = cream_rc_requirement[17] + smp_rc_smp_requirement[17] + smp_rc_wmp_requirement[17] + \
                         wmp_rc_wmp_requirement[17]
    ...
    cream_input_total[
        13] = cream_purchase_year + cream_stockin_year + cream_Jersey_production_year + cream_tm_production_year + cream_htm_production_year + cream_dtm_production_year + cream_sm_production_year + cream_surplus_csm_year
    cream_input_total[14] = cream_fat
    cream_input_total[15] = cream_purchase_list[15] + cream_stock_in_fat_year + cream_Jersey_production[15] + \
                            cream_tm_production[15] + cream_htm_production[15] + cream_dtm_production[15] + \
                            cream_sm_production[15] + cream_surplus_csm[15]
    cream_input_total[16] = cream_snf
    cream_input_total[17] = cream_purchase_list[17] + cream_stock_in_snf_year + cream_Jersey_production[17] + \
                            cream_tm_production[17] + cream_htm_production[17] + cream_dtm_production[17] + \
                            cream_sm_production[17] + cream_surplus_csm[17]
    ...
    cream_output_total[
        13] = cream_std_production_year + cream_rc_requirement_year + cream_stockout_year + cream_sale_list_year
    cream_output_total[14] = cream_fat
    cream_output_total[15] = cream_std_production[15] + cream_rc_requirement[15] + cream_stock_out_fat_year + \
                             cream_sale_list[15]
    cream_output_total[16] = cream_snf
    cream_output_total[17] = cream_std_production[17] + cream_rc_requirement[17] + cream_stock_out_snf_year + \
                             cream_sale_list[17]
    ...
    return render(request, 'prediction/ReportAnnualMAT.html',
                  {'wm_purchase_list': wm_purchase_list, 'wm_reconstitution_list': wm_reconstitution_list
                      , 'wm_requirement_product': wm_requirement_product, 'wm_stock_out_transfer_list': wm_stock_out,
                   'wm_procurement_list': wm_procurement_list
                      , 'wm_Jersey_production': wm_Jersey_production, 'wm_tm_production': wm_tm_production,
                   'wm_htm_production': wm_htm_production, 'wm_std_production': wm_std_production
                      , 'wm_dtm_production': wm_dtm_production, 'wm_sm_production': wm_sm_production,
                   'wm_surplus_sale_list': wm_surplus_sale_list
                      , 'month': month_list, 'wm_stock_in_transfer_list': wm_stock_in, 'wm_input_total': wm_input_total,
                   'wm_surplus_csm': wm_surplus_csm
                      , 'wm_output_total': wm_output_total, 'cream_std_production': cream_std_production,
                   'smp_std_production': smp_std_production
                      , 'std_input_total': std_input_total, 'total_std_requirement': total_std_requirement,
                   'std_output_total': std_output_total
                      , 'cream_tm_production': cream_tm_production, 'smp_tm_production': smp_tm_production,
                   'tm_input_total': tm_input_total, 'total_tm_requirement': total_tm_requirement
                      , 'tm_output_total': tm_output_total, 'cream_Jersey_production': cream_Jersey_production,
                   'smp_Jersey_production': smp_Jersey_production
                      , 'Jersey_input_total': Jersey_input_total, 'total_Jersey_requirement': total_Jersey_requirement,
                   'Jersey_output_total': Jersey_output_total
                      , 'cream_htm_production': cream_htm_production, 'smp_htm_production': smp_htm_production,
                   'total_htm_requirement': total_htm_requirement
                      , 'htm_input_total': htm_input_total, 'htm_output_total': htm_output_total,
                   'cream_dtm_production': cream_dtm_production, 'smp_dtm_production': smp_dtm_production
                      , 'total_dtm_requirement': total_dtm_requirement, 'dtm_input_total': dtm_input_total,
                   'dtm_output_total': dtm_output_total
                      , 'cream_sm_production': cream_sm_production, 'smp_sm_production': smp_sm_production,
                   'total_sm_requirement': total_sm_requirement,
                   'sm_input_total': sm_input_total, 'sm_output_total': sm_output_total,
                   'cream_rc_requirement': cream_rc_requirement, 'smp_rc_smp_requirement': smp_rc_smp_requirement
                      , 'smp_rc_wmp_requirement': smp_rc_wmp_requirement,
                   'water_rc_smp_requirement': water_rc_smp_requirement,
                   'water_rc_wmp_requirement': water_rc_wmp_requirement,
                   'wmp_rc_wmp_requirement': wmp_rc_wmp_requirement, 'rc_input_total': rc_input_total,
                   'rc_output_total': rc_output_total, 'cream_purchase_list': cream_purchase_list
                      , 'cream_stock_in': cream_stock_in, 'cream_surplus_csm': cream_surplus_csm,
                   'cream_input_total': cream_input_total, 'cream_requirement_product': cream_requirement_product
                      , 'cream_stock_out': cream_stock_out, 'cream_sale_list': cream_sale_list,
                   'cream_output_total': cream_output_total, 'smp_purchase_list': smp_purchase_list,
                   'smp_from_conversion': smp_from_conversion, 'smp_input_total': smp_input_total,
                   'smp_requirement': smp_requirement, 'smp_sale_list': smp_sale_list
                      , 'smp_output_total': smp_output_total, 'csm_produced': csm_produced,
                   'csm_input_total': csm_input_total, 'csm_sale_list': csm_sale_list,
                   'csm_for_conversion': csm_for_conversion
...
def reportFinanceYear(request):
    ...
    print category_sale_list

    wm_purchase_monthlist = OrderedDict()
    cream_purchase_monthlist = OrderedDict()
    smp_purchase_monthlist = OrderedDict()
    wmp_purchase_monthlist = OrderedDict()
    wm_purchase_diary_list = OrderedDict()
    cream_purchase_diary_list = OrderedDict()
    smp_purchase_diary_list = OrderedDict()
    wmp_purchase_diary_list = OrderedDict()
    total_input_diary_list = OrderedDict()
    total_output_diary_list = OrderedDict()
    ...
    for month_item in MONTHS.items():
        ...
        for item_smp in smp_requirement_list:
            ...
            if item_smp['type'] == "Sale":
                ...
                try:
                    ...
                    smp_purchase_list.append(smp_purchase_item)
        ...
        for item_wmp in wmp_requirement_list:
            ...
            if item_wmp['type'] == "Sale":
                ...
                try:
                    rate = GeneralCalculation.objects.get(code='11').value
                    wmp_purchase_amount = item_wmp['total_wmp_used'] * rate
                    wmp_item = {'transaction': 'WMP Purchased:' + str(
                        item_wmp['total_wmp_used']) + " ,Amount:" + str(wmp_purchase_amount),
                                'diary': item_wmp['diary'].name}
                    wmp_purchase_item = {'diary': item_smp['diary'].name, 'Amount': wmp_purchase_amount}
                    wmp_purchase_list.append(wmp_purchase_item)
        ...
        wmp_purchase_monthlist[month] = wmp_purchase_list
    ...
    for diary in diaryList:
        ...
        wm_purchase_diary = 0
        for wm_purchase in wm_purchase_monthlist.items():
            for item in wm_purchase[1]:

                if diary.name == item['diary']:
                    wm_purchase_diary += item['Amount']
        wm_purchase_diary_list[diary.name] = wm_purchase_diary

        cream_purchase_diary = 0
        for cream_purchase in cream_purchase_monthlist.items():

            for cream_item in cream_purchase[1]:
                if diary.name == cream_item['diary']:
                    cream_purchase_diary += cream_item['Amount']
            cream_purchase_diary_list[diary.name] = cream_purchase_diary

        smp_purchase_diary = 0
        for smp_purchase in smp_purchase_monthlist.items():

            for smp_item in smp_purchase[1]:
                if diary.name == smp_item['diary']:
                    smp_purchase_diary += smp_item['Amount']
            smp_purchase_diary_list[diary.name] = smp_purchase_diary

        wmp_purchase_diary = 0
        for wmp_purchase in wmp_purchase_monthlist.items():

            for wmp_item in wmp_purchase[1]:
                if diary.name == wmp_item['diary']:
                    wmp_purchase_diary += wmp_item['Amount']
            wmp_purchase_diary_list[diary.name] = wmp_purchase_diary

        total_input_diary_list[
            diary.name] = wm_purchase_diary + cream_purchase_diary + smp_purchase_diary + wmp_purchase_diary + wm_procurement_diary

        category_sum = 0
        for category_item in category_sale_list.items():

            for category_diary in category_item[1]:
                if diary.name == category_diary['diary']:
                    category_sum += category_diary['amount']
        total_output_diary_list[diary.name] = category_sum

    return render(request, 'prediction/ReportFinance.html',
                  {'diaryList': diaryList, 'wm_procurement_list': wm_procurement_list,
                   'category_sale_list': category_sale_list, 'wm_purchase_diary_list': wm_purchase_diary_list,
                   'cream_purchase_diary_list': cream_purchase_diary_list,
                   'smp_purchase_diary_list': smp_purchase_diary_list
                      , 'wmp_purchase_diary_list': wmp_purchase_diary_list,
                   'total_input_diary_list': total_input_diary_list
                      , 'total_output_diary_list': total_output_diary_list})


@login_required
@user_passes_test(group_check_diary)
def reportFinanceMonth(request):
    if request.method == "POST":
        form = MonthOnlyForm(request.POST)
        ...
        if form.is_valid():
            ...
            for category in categoryList:
                ...
                for diary in diaryList:
                    ...
                    saledetails = ActualSale.objects.filter(
                        product__in=(Product.objects.filter(category=category)).values('code'), month=month,
                        diary=diary)
            ...
            after_cream_balancing_diarylist, cream_purchase_list = creamBalancingReportFinance(
                after_cream_transfer_diary)
            cream_purchase_monthlist[month] = cream_purchase_list
            ...
            for diary in diaryList:
                ...
                try:
                    ...
                    wm_procurement_obj = ActualWMProcurement.objects.get(diary=diary, month=month)
                    wm_procurement_month = wm_procurement_obj.targetAmount
                except Exception as e:
                    print  "Exception handeld in reportFinanceYear wm procurement fetching failed"
                    wm_procurement_month = 0
            ...
            return render(request, 'prediction/ReportFinanceMonthly.html',
                          {'form': form, 'diaryList': diaryList, 'wm_procurement_list': wm_procurement_list,
                           'category_sale_list': category_sale_list, 'wm_purchase_diary_list': wm_purchase_diary_list,
                           'cream_purchase_diary_list': cream_purchase_diary_list,
                           'smp_purchase_diary_list': smp_purchase_diary_list
                              , 'wmp_purchase_diary_list': wmp_purchase_diary_list,
    ...
    return render(request, 'prediction/ReportFinanceMonthly.html', {'form': form})


@login_required
@user_passes_test(group_check_union)
def reportFinanceYearUnion(request):
    categoryList = Category.objects.all()
    diaryList = Diary.objects.all()
    wm_procurement_list = OrderedDict()
    category_sale_list = OrderedDict()
    ...
    return render(request, 'prediction/ReportFinanceUnion.html',
                  {'diaryList': diaryList, 'wm_procurement_list': wm_procurement_list,
                   'category_sale_list': category_sale_list, 'wm_purchase_diary_list': wm_purchase_diary_list,
                   'cream_purchase_diary_list': cream_purchase_diary_list,
                   'smp_purchase_diary_list': smp_purchase_diary_list
...
@login_required
@user_passes_test(group_check_union)
def reportFinanceMonthUnion(request):
    if request.method == "POST":
        form = MonthOnlyForm(request.POST)
        ...
        if form.is_valid():
            ...
            return render(request, 'prediction/ReportFinanceMonthlyUnion.html',
                          {'form': form, 'diaryList': diaryList, 'wm_procurement_list': wm_procurement_list,
                           'category_sale_list': category_sale_list, 'wm_purchase_diary_list': wm_purchase_diary_list,
                           'cream_purchase_diary_list': cream_purchase_diary_list,
                           'smp_purchase_diary_list': smp_purchase_diary_list
    ...
    return render(request, 'prediction/ReportFinanceMonthlyUnion.html', {'form': form})
